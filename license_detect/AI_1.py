...
def channl_open(cam,ai_id,thresh,channel,area,qu_plate,qu_org):
        ...
        while True:
            ...
            text,plate, img_r = detect(img,area)  #AI辨識  return 車牌號碼/車牌縮圖
   
            for i, plate_num in enumerate(text):
                if len(plate_num) > 4:
                    timestart += temptime
                    if not any(plate_num in s for s in List):
                        List.append([plate_num, 0])
                        plate_list.append(plate_num)
                    else:
                        columns = list(zip(*List))
                        post = columns[0].index(plate_num)
                        List[post][1] += 1
            ...
            if temp != incar and len(incar)>4:   #看門檻是否與上筆重覆                
                ...
                if len(plate) > 0:
                    for i in range(len(plate)):
                        # 修改存檔的檔名，加入車牌編號
                        cv2.imwrite(r'C:/Users/Asc-user/Documents/YOLO/darknet/mainpicture/'+plate_list[i]+'.png',plate[i])
                        msg = cv2.imencode(".jpg", plate[i], [cv2.IMWRITE_JPEG_QUALITY, qu_plate])[1]
                        plate[i] = (np.array(msg)).tobytes()
            ...
            temptime = end- start       
            img_r = cv2.resize(img_r, (h//3, w//3), interpolation=cv2.INTER_CUBIC)               
