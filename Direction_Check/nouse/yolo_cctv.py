...
def detect(img, area):
    ...
    #只取coco列表中2汽車7卡車，有需要再加 3機車
    valid_classes = [classes[2], classes[7]]
    ...
    if len(detections) > 0:
        for label, confidence, bbox in detections: #每一個被偵測到的物件
            if label in valid_classes:
                ...
                check_stopped(center_x, center_y) #要排除非移動車，目前看沒作用
...
async def check_stopped(center_x, center_y):
    # 記錄前一次的座標
    prev_x = center_x
    prev_y = center_y
    # 設定連續判斷的次數
    ...
    async def check_loop():
        ...
        for i in range(num_checks):
            await asyncio.sleep(0.1)  # 等待0.1秒，避免過快地進行連續判斷
            curr_x, curr_y = center_x, center_y  # 取得目前的座標
            print("curr_x",curr_x)
            if abs(curr_x - prev_x) < 3 and abs(curr_y - prev_y) < 3:
                results.append(True)  # 如果是，則放入True
            else:
                results.append(False)  # 如果不是，則放入False
            # 更新前一次的座標
            prev_x = curr_x
            prev_y = curr_y
            print("prev_x",prev_x)
    ...
    # 檢查所有判斷結果是否都為True
    if all(results):
        return None  # 如果都是True，則代表車子已經停下來，回傳None
    else:
        return center_x, center_y  # 如果有任何一次判斷結果為False，則代表車子沒有停下來，回傳座標值
