...
style.configure("TLabel", background="gray70", font=("Arial", 20, "bold"))# 設定標籤字體大小
style.configure("TButton", font=("Arial", 20, "bold"))  # 設定按鈕字體大小
style.configure("Treeview.Heading", font=("Arial", 20, "bold"))  # 設定表格標題字體大小
style.configure("Treeview", font=("Arial", 20, "bold"))  # 設定表格內文字字體大小
...
# 建立上方查詢欄位框架
query_frame = ttk.Frame(main_frame, padding=10)
query_frame.pack()
vcmd = (query_frame.register(validate), '%P')
query_entry = ttk.Entry(query_frame, width=40, font=("Arial", 20), validate='key', validatecommand=vcmd)
...
condition_button.pack(side=tk.LEFT, padx=5)
# 放大圖片按鈕
enlarge_button = ttk.Button(query_frame, text="放大圖片")
enlarge_button.pack(side=tk.LEFT, padx=5)
# 設定查詢欄位框架的寬度
...
widths = [30, 240, 150, 150, 30]  # 自定義的欄位寬度
...
renew_button.grid(row=0, column=0, padx=5, pady=5)
Now_Page = "當前頁數"
Total_Page = "總頁數"
total_pages_label = ttk.Label(bottom_frame, text= Now_Page + "/" + Total_Page)
total_pages_label.grid(row=0, column=1, padx=5, pady=5)
...
# 調整主視窗的位置
root.update_idletasks()  # 確保元件大小已被計算
screen_width = root.winfo_screenwidth()
screen_height = root.winfo_screenheight()
window_width = root.winfo_width()
window_height = root.winfo_height()
x_pos = (screen_width - window_width) // 2
y_pos = (screen_height - window_height) // 2 - 30  # 提升視窗位置30個像素(依工具列調整)
root.geometry(f"{window_width}x{window_height}+{x_pos}+{y_pos}")
# 建立顯示img的框架 1.隨便放圖 2.現場影像 3.cam1全圖 4.cam1噴字特寫
img_frame1 = Canvas(bottom_frame, width=450, height=350, relief=tk.RAISED, borderwidth=1)
img_frame1.grid(row=1, column=0, padx=10, pady=10)
img_frame2 = Canvas(bottom_frame, width=450, height=350, relief=tk.RAISED, borderwidth=1)
img_frame2.grid(row=1, column=1, padx=10, pady=10)
img_frame3 = Canvas(bottom_frame, width=450, height=350, relief=tk.RAISED, borderwidth=1)
img_frame3.grid(row=1, column=2, padx=10, pady=10)
img_frame4 = Canvas(bottom_frame, width=450, height=350, relief=tk.RAISED, borderwidth=1)
img_frame4.grid(row=1, column=3, padx=10, pady=10)
